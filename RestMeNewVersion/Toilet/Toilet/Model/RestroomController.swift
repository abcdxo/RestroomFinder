...
class RestroomController {
  ...
  static let sharedInstance = RestroomController()
  private var baseURL = URL(string: "https://www.refugerestrooms.org/api")!
  ...
  func createRestroom( name: String, street: String, city: String, state: String, country: String,longitude:Float,latitude:Float)  {
    ...
    if favoriteRestrooms.contains(restroom) {
      NotificationCenter.default.post(name: .contained, object: self)
    } else {
      favoriteRestrooms.append(restroom)
      NotificationCenter.default.post(name: .success, object: self)
    }
    saveToPersistStore()
  }
  ...
  func deleteEvent(restroom: Restroom) {
    ...
    guard let indexOfRestroomToDelete = favoriteRestrooms.firstIndex(of: restroom) else { return }
    favoriteRestrooms.remove(at: indexOfRestroomToDelete)
    saveToPersistStore()
  }

  // MARK:- Networking     https://www.refugerestrooms.org/api/v1/restrooms/search?page=1&per_page=10&offset=0&ada=true&unisex=true&query=London
  //  https://www.refugerestrooms.org/api/v1/restrooms?page=1&per_page=10&offset=0&ada=true&unisex=true&query=London

  func refetchRestroom(lat:Double,long:Double, completion: @escaping ([Restroom]?,Error?) -> Void ) {
    var baseURLL = URL(string: "https://www.refugerestrooms.org/api")!
    baseURLL.appendPathComponent("v1/restrooms/by_location")
    let urlComponents = NSURLComponents(url: baseURLL, resolvingAgainstBaseURL: false)
